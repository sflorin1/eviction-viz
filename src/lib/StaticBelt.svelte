...
<script>
  ...
  // SVG path for the zoom out view
  const zoomOutSvgPath = 'ZoomOut.svg';
  ...
  let showMessage = false; // Flag to show the message after zoom out
  ...
  scrollY.subscribe(value => {
    ...
    // Zoom out phase
      ...
      // Show message when zoom out is complete but before the map section
      // Only show it in a specific scroll range
      const messageStartScroll = zoomOutEnd - 300; // Show message near end of zoom
      const messageEndScroll = zoomStartPosition + BOX_Y_SCROLL - 200; // Hide before map appears
      showMessage = value >= messageStartScroll && value < messageEndScroll;
    }
    ...
    debugInfo.showMessage = showMessage;
  });
...
>
  ...
  {#if showAllRows && currentScroll >= zoomStartPosition && currentScroll < (zoomStartPosition + BOX_Y_SCROLL)}
    ...
    <!-- Phase 2: Vertical line SVGs - Changed to use SVG instead of PNG -->
    <div class="svg-container" style="opacity: {calculateFrameOpacityBoxes(currentScroll)};">
      <img src={zoomOutSvgPath} alt="Zoomed Out View">
    </div>
  {/if}
</div>
...
<!-- Black rectangle message after zoom out is complete -->
{#if showMessage}
  <div class="message-container" transition:fade={{ duration: 800 }}>
    <div class="message-box">
      <p>Eviction Filings like this happen in the thousands, let's have a closer look</p>
    </div>
  </div>
{/if}
...
{#if debug}
  <div class="debug-info">
    ...
    <p>Message: {debugInfo.showMessage ? 'Visible' : 'Hidden'}</p>
